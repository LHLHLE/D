...
stripe.api_key = "sk_test_51N2gSnSGHy0wWvvk16uP2r9iCslBALr75yp1PU33rwwJWe8Mw1OFO6CROfzf9gn8VGmSaMSf9HgMsowwyik7HD36001Tx3RztI"
            ...
            line_items=[
                {
                    ...
                    'price': 'price_1N2gZlSGHy0wWvvksdpqr9o3',
...
def cred_suc():
    ...
    return home("Journal Successfully Revived")
...
def return_records():
    ...
    emails = get_lists_of_emails_api(database=db, term=keyword, count=count, start=start, from_date=from_date, to_date=to_date)
    if emails[0] == {}:
...
def add_listing(db_id):
    ...
    with open(filename, 'a') as file:
            with open(filename, 'a', newline="") as file:
                writer = csv.writer(file)
                new_row = [name, email, db_id, session["user_id"]]
                writer.writerow(new_row)
    filename = "email_names.csv"
    with open(filename, 'r') as file:
            reader = csv.reader(file)
...
def export_listing(db_id):
    ...
    sheet.write(0, 2, 'From Date')
    sheet.write(0, 3, 'To Date')
...
def journals(message=False, journals=False, no_of_jour=False):
    if 'user_id' in session:    
        ...
        if session['user_id'] in emails:
            ...
            for row in data[1:]:
                if row[0] == session['user_id']:
                    journals.append([row[2], row[3], row[4], row[5], row[6], row[7], row[1]])
...
def receiver():
    if 'user_id' in session:
        ...
        '''
        try:
            send_email(from_email, from_pass, "pandeyrainy2020@gmail.com", subject, "Testing", hosting, port)
        except Exception as e:
...
if __name__ == '__main__':
    app.run(threaded=True, debug=True, port=1250)
